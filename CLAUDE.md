...
> **History:** This was a 2-on-1 podcast platform until the 2026-05 events pivot. The podcast public routes (`/episodes`, `/guests`, `/guest`, `/fireside`, `/apply`) were removed; the homepage and `/about` were reframed for events; a public `/events` page and an `/admin/events` section were added. The podcast `episodes` table, its admin pages, and the transcript-based AI clip-suggestions feature were then fully removed; `ai_runs.episode_id` was dropped. The `aiRunKindEnum` still lists the now-unused `summarize_transcript` / `clip_suggestions` values (Postgres enum values are painful to drop; harmless). The CRM (contacts, pipeline, outreach, AI enrich/draft, submissions) is unchanged.
...
- **Server Actions**: `src/lib/actions/{contacts,events,outreach,submissions,ai}.ts`. Every action calls `await auth()` and throws if `!userId`. Actions revalidate the relevant paths so server components reflect changes. `events.ts` also revalidates the public `/` and `/events`.
